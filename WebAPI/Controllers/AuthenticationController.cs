...
namespace WebAPI.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public AuthenticationController(IUnitOfWork unitOfWork, IMailService mail, IHubContext<BroadcastHub, IHubClient> hubContext)
        {
        ...
        public IActionResult Login([FromBody] Login user)
        {
            ...
            if (obj != null)
            {
                ...
                {
                    var myExpns = _unitOfWork.Group.GetT(x => x.AddedBy == obj.Id && x.GroupName == "My Expense");
                    var claims = new[]{
                                        new Claim("Id",obj.Id.ToString()),
                                        new Claim("Name",obj.Name),
                                        new Claim("Email",obj.Email),
                                        new Claim("TokenFor","UserAuth"),
                                        new Claim("MyExpense", myExpns.Result.Id.ToString())
                                        };
                    ...
                    Notification notification = new Notification()
                    {
                        ...
                        Details=obj.Name+"! has made login Now.",
                        IsDeleted=false,
                        IsRead=false,
                        IsReminder=false,
                    ...
                    _unitOfWork.Notification.Insert(notification);
                    _unitOfWork.Save();
        ...
        public async Task<IActionResult> ForgotPassword([FromQuery] string EmailId)
        {
            ...
            {
                UserDetail obj = await _unitOfWork.User.GetT(x => x.Email == EmailId && x.IsActive == true && x.IsDeleted == false);
                if (obj == null)
                {
        ...
        public async Task<IActionResult> GetEmailByToken()
        {
            ...
            if (identity != null)
            {
                ...
                if (UserId!=null)
                {
                    return Ok(new APIResponse { StatusCode = StatusCodes.Status200OK.ToString(), Response = (await _unitOfWork.User.GetT(x => x.Id == UserId)).Email.ToString(), Status = "Success" });
                }
                else
                {
