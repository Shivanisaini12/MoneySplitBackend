...
namespace WebAPI.Controllers
{
    ...
    public class GroupMemberController : ControllerBase
    {
            ...
            try
            {
                ...
                if (identity != null && lstGroupMember != null && lstGroupMember.Count > 0)
                {
                    ...
                    foreach (var member in lstGroupMember)
                    {
                        ...
                        var existUser = await _unitOfWork.User.GetT(x => x.Email == mappedMember.Email);
                        var existUserInGroup = await _unitOfWork.GroupMember.GetT(x => x.Email == mappedMember.Email && x.GroupId == GroupId);
                        ...
                        else
                        {
                            ...
                            List<Notification> addMemberNoti = new List<Notification>();
                            List<Notification> allNoti = new List<Notification>();
                            addMemberNoti = _unitOfWork.Notification.AddMemberNotification(GroupId,member.Email);
                            allNoti = _unitOfWork.Notification.GetNotifications(UserId);

                            messageHub.Clients.All.SendMemberAdd(addMemberNoti);
        ...
        public async Task<IActionResult> DeleteGroupMember(int memberId)
        {
            var userClaims = (HttpContext.User.Identity as ClaimsIdentity).Claims;
            Guid UserId = new Guid(userClaims.FirstOrDefault(x => x.Type == "Id").Value);

            var groupMember = await _unitOfWork.GroupMember.Get(memberId);
            if (groupMember != null)
            {
                ...
                if (res)
                {
                    ...
                    List<Notification> expenseNoti = new List<Notification>();
                    List<Notification> allNoti = new List<Notification>();

                  
                    expenseNoti = _unitOfWork.Notification.LeaveGroupNotification(memberId, UserId);

                    allNoti = _unitOfWork.Notification.GetNotifications(UserId);
                    messageHub.Clients.All.SendLeaveToUser(expenseNoti);
                    messageHub.Clients.All.sendAllNotifications(allNoti);
